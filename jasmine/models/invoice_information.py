...
################################################################################################################
    # 汎宇API狀態追蹤欄位
    fanyuu_status = fields.Selection([
        ('draft', '草稿'),
        ('sent', '已送出'),
        ('success', '成功'),
        ('failed', '失敗')
    ], string='汎宇狀態', default='draft', tracking=True)
    fanyuu_message = fields.Text(string='汎宇回應訊息', readonly=True)
    fanyuu_sent_time = fields.Datetime(string='送出時間', readonly=True)
    fanyuu_response_time = fields.Datetime(string='回應時間', readonly=True)
    ...
    def action_send_to_fanyuu(self):
        ...
        for record in self:
            try:
                ...
                record.write({
                    'fanyuu_status': 'sent',
                    'fanyuu_sent_time': fields.Datetime.now()
                })
                
                # 檢查API類型設定
                api_type = self.env['ir.config_parameter'].sudo().get_param('fanyuu.api_type', 'pos_json')
                
                if api_type == 'pos_json':
                    # 使用 POS機 JSON 格式
                    result = self._send_to_fanyuu_pos_api(record)
                else:
                    # 使用原有的 REST API 格式
                    from odoo.addons.jasmine.services.fanyuu_api import FanyuuAPI
                ...
                # 處理回應結果
                record._handle_fanyuu_response(result)
                
            except Exception as e:
                # 錯誤處理
                record.write({
                    'fanyuu_status': 'failed',
                    'fanyuu_message': str(e),
                    'fanyuu_response_time': fields.Datetime.now()
                })
                _logger.error(f"發送發票到汎宇失敗: {str(e)}")
                raise UserError(f"發送發票到汎宇失敗: {str(e)}")
        
        return True
    
    # 使用POS機JSON格式發送到汎
    def _send_to_fanyuu_pos_api(self, invoice_info):
        """使用POS機JSON格式發送到汎宇"""
        ...
        # 根據操作類型選擇不同的API方法
        if invoice_info.operation_type == 'CAN':
            # 作廢發票
            result = api.cancel_invoice(invoice_info)
        elif invoice_info.operation_type == 'CRE_ALW':
            # 開立折讓
            result = api.create_allowance(invoice_info)
        else:
            # 開立發票
            result = api.create_invoice(invoice_info)
        
        return result
    
    #處理汎宇API回應
    def _handle_fanyuu_response(self, result):
        """處理汎宇API回應"""
        try:
            success = False
            message = ""
            ...
            # 更新狀態
            self.write({
                'fanyuu_status': 'success' if success else 'failed',
                'fanyuu_message': message,
                'fanyuu_response_time': fields.Datetime.now()
            })
            ...
            if success:
                # 發送成功通知
                self.message_post(
                    body=f"電子發票已成功送出至汎宇系統<br/>回應訊息: {message}",
                    message_type='notification'
                )
            else:
                raise UserError(f"汎宇系統回應錯誤: {message}")
                
        except Exception as e:
            _logger.error(f"處理汎宇回應時發生錯誤: {str(e)}")
            self.write({
                'fanyuu_status': 'failed',
                'fanyuu_message': f"處理回應時發生錯誤: {str(e)}",
                'fanyuu_response_time': fields.Datetime.now()
            })
    ...
    def action_create_cancellation(self):
        ...
        # 檢查發票狀態
        if self.operation_type in ['CAN', 'REJ']:
            raise UserError('此發票已被作廢或退回，不能再次作廢！')
        
        # 創建作廢單
        cancellation_vals = {
            'invoice_id': self.id,
            'cancellation_reason': 'wrong_info',
            'reason_detail': '發票作廢',
        }
        
        cancellation = self.env['invoice.cancellation'].create(cancellation_vals)
    ...
    def sync_credit_note_from_account_move(self, credit_note_move):
        ...
        if not original_invoice_info:
            _logger.warning(f"找不到貸記單 {credit_note_move.name} 對應的原始發票電子發票資訊")
            return
        ...
        # 自動上傳到汎宇系統
        try:
            allowance_info._send_allowance_to_fanyuu(credit_note_move, original_invoice_info)
            _logger.info(f"折讓證明單 {allowance_info.invoice_number} 已成功上傳到汎宇系統")
        except Exception as e:
            _logger.error(f"上傳折讓證明單到汎宇失敗: {str(e)}", exc_info=True)
            # 不中斷流程，但記錄錯誤
            allowance_info.with_context(force_einvoice_edit=True).write({
                'fanyuu_status': 'failed',
                'fanyuu_message': f"自動上傳失敗: {str(e)}",
                'fanyuu_response_time': fields.Datetime.now()
            })
            
            # 發送系統通知
            allowance_info.message_post(
                body=f"折讓證明單自動上傳到汎宇系統失敗<br/>"
                     f"錯誤訊息: {str(e)}<br/>"
                     f"請稍後手動重新上傳",
                message_type='notification'
            )
    ...
    # 發送折讓證明單到汎宇系統 (D0401)
    def _send_allowance_to_fanyuu(self, credit_note_move, original_invoice_info):
        """發送折讓證明單到汎宇系統 (D0401)"""
        try:
            from odoo.addons.jasmine.services.fanyuu_pos_api import FanyuuPosAPI
            ...
            _logger.info(f"準備上傳折讓證明單到汎宇: {self.invoice_number}")
            ...
            for line in credit_note_move.invoice_line_ids.filtered(lambda l: l.display_type not in ('line_section', 'line_note')):
                ...
                items.append({
                    'original_invoice_date': original_invoice_info.invoice_date.strftime('%Y-%m-%d') if original_invoice_info.invoice_date else '',
                    'original_invoice_number': original_invoice_info.invoice_number,
                    'original_sequence_number': line_seq,
                    'original_description': line.product_id.name or line.name or '',
                    'quantity': abs(line.quantity),
                    'unit': line.product_uom_id.name if line.product_uom_id else '',
                    'unit_price': abs(line.price_unit),
                    'amount': abs(line.price_subtotal),
                    'tax': abs(line.price_total - line.price_subtotal) if line.price_total != line.price_subtotal else 0,
                    'tax_type': 1  # 應稅
                })
                line_seq += 1
            
            if not items:
                raise UserError("沒有有效的折讓明細項目")
            
            # 取得發票抬頭ID
            invoice_title_id = None
            if hasattr(self, 'invoice_track_id') and self.invoice_track_id:
                if hasattr(self.invoice_track_id, 'invoice_title_id'):
                    invoice_title_id = self.invoice_track_id.invoice_title_id.id
            
            # 準備折讓資料
            allowance_data = {
                'allowance_number': self.invoice_number,
                'buyer_id': credit_note_move.partner_id.vat or '0000000000',
                'buyer_name': credit_note_move.partner_id.name or '',
                'buyer_address': self._get_partner_address(credit_note_move.partner_id),
                'buyer_phone': credit_note_move.partner_id.phone or '',
                'buyer_email': credit_note_move.partner_id.email or '',
                'buyer_customer_number': credit_note_move.name,
                'tax_amount': abs(int(credit_note_move.amount_tax)),
                'total_amount': abs(int(credit_note_move.amount_untaxed)),
                'original_invoice_seller_id': original_invoice_info.seller_tax_id,
                'original_invoice_buyer_id': original_invoice_info.buyer_tax_id or '',
                'items': items
            }
            
            _logger.info(f"折讓資料準備完成，明細項目數: {len(items)}")
            
            # 發送到汎宇 (D0401)
            result = api.create_allowance(allowance_data, invoice_title_id)
            
            _logger.info(f"汎宇折讓證明單回應結果: {result}")
            
            if result.get('success'):
                # 更新狀態為成功
                self.with_context(force_einvoice_edit=True).write({
                    'fanyuu_status': 'success',
                    'fanyuu_message': result.get('message', '折讓證明單上傳成功'),
                    'fanyuu_response_time': fields.Datetime.now()
                })
                
                # 記錄成功訊息
                self.message_post(
                    body=f"折讓證明單已成功上傳到汎宇系統<br/>"
                         f"折讓證明單號: {self.invoice_number}<br/>"
                         f"原始發票號碼: {original_invoice_info.invoice_number}<br/>"
                         f"貸記單號: {credit_note_move.name}<br/>"
                         f"回應訊息: {result.get('message', '')}",
                    message_type='notification'
                )
                
                _logger.info(f"折讓證明單 {self.invoice_number} 已成功上傳到汎宇系統")
                ...
                self.with_context(force_einvoice_edit=True).write({
                    'fanyuu_status': 'failed',
                    'fanyuu_message': f"上傳失敗: {error_message}",
                    'fanyuu_response_time': fields.Datetime.now()
                })
            ...
            error_msg = f"上傳折讓證明單到汎宇失敗: {str(e)}"
    ...
    #  重新上傳折讓證明單到汎宇系統
    def action_retry_allowance_fanyuu_upload(self):
        """重新上傳折讓證明單到汎宇系統"""
        self.ensure_one()
        
        if self.operation_type != 'CRE_ALW':
            raise UserError('只有折讓證明單才能使用此功能！')
        ...
        if not credit_note:
            raise UserError('找不到對應的貸記單！')
        
        # 找到原始發票資訊
        original_invoice_info = self.search([
            ('account_move_ids', 'in', credit_note.reversed_entry_id.ids)
        ], limit=1)
        
        if not original_invoice_info:
            raise UserError('找不到原始發票資訊！')
        
        try:
            self._send_allowance_to_fanyuu(credit_note, original_invoice_info)
            ...
            raise UserError(f"重新上傳折讓證明單失敗: {str(e)}")
